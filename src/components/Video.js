...
function Video() {
    ...
    const [vid, setvied] = useState(videosArr[0])
    return (
        <Stack direction={["column", "row"]} h={"100vh"}>
            <VStack w={"full"}>
                <video controls controlsList='nodownload' src={vid} style={{ width: '100%', }}></video>
                <VStack alignItems={"flex-start"} p={"8"} w={"full"} overflowY={"auto"}>
                    <Heading>Sample video 1</Heading>
                    <Text>This is a sample video for testing and demo.this is discription</Text>
                </VStack>
            </VStack>
            <VStack w={["full", "xl"]} alignItems={"stretch"} p={"8"} spacing={"8"} overflowY={"auto"}>
                {
                    videosArr.map((ele,ind)=>{
                        return <Button variant={"ghost"} colorScheme={'purple'} onClick={()=>setvied(ele)}>Lecture {ind+1}</Button>
                    })
                }
            </VStack>
        </Stack>
